...
class MobileNetV3MultiTaskAgeGender(nn.Module):
    def __init__(self, num_age_groups=len(AGE_GROUPS), num_genders=len(GENDERS)):
        super(MobileNetV3MultiTaskAgeGender, self).__init__()
        self.backbone = mobilenet_v3_small(weights=None) 
        num_ftrs = self.backbone.classifier[0].in_features
        self.backbone.classifier = nn.Identity()
        age_intermediate_features = 256
        gender_intermediate_features = 128
        self.age_classifier = nn.Sequential(
            nn.Linear(num_ftrs, age_intermediate_features), 
            nn.Hardswish(),                                 
            nn.Dropout(p=0.2, inplace=True),                
            nn.Linear(age_intermediate_features, num_age_groups)
        )
        self.gender_classifier = nn.Sequential(
            nn.Linear(num_ftrs, gender_intermediate_features),
            nn.Hardswish(),                                  
            nn.Dropout(p=0.2, inplace=True),                 
            nn.Linear(gender_intermediate_features, num_genders)
        )
    def forward(self, x):
...
def main():
    ...
    try:
        ...
        age_gender_model = MobileNetV3MultiTaskAgeGender()
        try:
...
if __name__ == "__main__":
    ...
    print("Ensure your model files (yolo11n.pt, yolov11n-face.pt, mobilenetv3_small_multi_task_best.pth) are in the 'weight' directory.")
    main()

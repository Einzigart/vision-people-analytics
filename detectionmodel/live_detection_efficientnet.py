...
# Try to import timm for EfficientNet-Lite, fallback to torchvision
try:
    import timm
    USE_TIMM = True
except ImportError:
...
AGE_GENDER_MODEL_PATH = 'weight/efficientnet_liteb0_multi_task_best.pth' 
WEBCAM_ID = 0
...
class EfficientNetB0MultiTaskAgeGender(nn.Module):
    def __init__(self, num_age_groups=len(AGE_GROUPS), num_genders=len(GENDERS)):
        super(EfficientNetB0MultiTaskAgeGender, self).__init__()
        
        # Use timm's EfficientNet-Lite B0 if available, otherwise use a custom implementation
        if USE_TIMM:
            self.backbone = timm.create_model('efficientnet_lite0', pretrained=False, num_classes=0)
            num_ftrs = self.backbone.num_features
        else:
            # Create a custom backbone that matches the saved model structure
            self.backbone = self._create_custom_backbone()
            num_ftrs = 1280  # EfficientNet-Lite B0 output features
        
        # Updated architecture to match the saved model weights
        # Age classifier: 1280 -> 640 -> 320 -> 128 -> 6 (with activations and dropout)
        self.age_classifier = nn.Sequential(
            nn.Linear(num_ftrs, 640),      # Index 0
            nn.SiLU(),                     # Index 1
            nn.Dropout(p=0.2, inplace=True),  # Index 2
            nn.Linear(640, 320),           # Index 3
            nn.SiLU(),                     # Index 4
            nn.Dropout(p=0.2, inplace=True),  # Index 5
            nn.Linear(320, 128),           # Index 6
            nn.SiLU(),                     # Index 7
            nn.Dropout(p=0.2, inplace=True),  # Index 8
            nn.Linear(128, num_age_groups) # Index 9
        )
        
        # Gender classifier: 1280 -> 320 -> 128 -> 64 -> 2 (with activations and dropout)
        self.gender_classifier = nn.Sequential(
            nn.Linear(num_ftrs, 320),      # Index 0
            nn.SiLU(),                     # Index 1
            nn.Dropout(p=0.2, inplace=True),  # Index 2
            nn.Linear(320, 128),           # Index 3
            nn.SiLU(),                     # Index 4
            nn.Dropout(p=0.2, inplace=True),  # Index 5
            nn.Linear(128, 64),            # Index 6
            nn.SiLU(),                     # Index 7
            nn.Dropout(p=0.2, inplace=True),  # Index 8
            nn.Linear(64, num_genders)     # Index 9
        )
    
    def _create_custom_backbone(self):
        """Create a custom backbone that matches the saved model's naming convention"""
        # This is a simplified version - you might need to adjust based on the exact model used
        from torchvision.models import efficientnet_b0
...
def main():
    ...
    try:
        ...
        age_gender_model = EfficientNetB0MultiTaskAgeGender()
        try:
...
if __name__ == "__main__":
    ...
    print("Ensure your model files (yolo11n.pt, yolov11n-face.pt, efficientnet_liteb0_multi_task_best.pth) are in the 'weight' directory.")
    main()
